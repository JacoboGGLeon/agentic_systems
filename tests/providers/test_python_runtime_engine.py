"""PythonRuntimeEngine local execution and output-first ergonomics."""
...
from agentic_systems.providers.python_runtime import PythonRuntimeEngine
...
def test_python_runtime_returns_helpful_failure_for_natural_language_multi_tool_prompt() -> None:
    agent = Agent(name="calculator", tools=[sumar, multiplicar], engine="python-runtime")
...
def test_system_agent_can_select_python_runtime_engine_with_runtime_skill() -> None:
    system = build_system()
    ...
    assert isinstance(system._engine("python-runtime"), PythonRuntimeEngine)
    assert result.ok is True
    assert result.data["result"] == 42
    assert result.data["tool"] == "restar"


def test_direct_agent_async_python_runtime_run() -> None:
    agent = Agent(name="calculator", tools=[sumar], engine="python-runtime")
