...
This is the canonical home for the local tool-execution path. The legacy
``agentic_systems.engines.python_direct`` module re-exports this provider for
runtime stability while the codebase finishes the migration.
"""
...
class PythonDirectProvider:
    ...
    def run(self, agent: Any, input: Any, policy: RunPolicy, *, mode: str = "default") -> RunResult:
        ...
        if not tools:
            return _failure(
                message="PythonDirectProvider needs at least one concrete Tool on the agent.",
                agent=agent,
                mode=mode,
                code="missing_tools",
        ...
        try:
            ...
            if pipeline is not None:
                ...
                if policy.max_tool_calls is not None and len(names) > policy.max_tool_calls:
                    return _failure(
                        message=f"PythonDirectProvider planned {len(names)} tool calls, above max_tool_calls={policy.max_tool_calls}.",
                        agent=agent,
                        mode=mode,
        ...
        if policy.max_tool_calls is not None and len(calls) > policy.max_tool_calls:
            return _failure(
                message=f"PythonDirectProvider planned {len(calls)} tool calls, above max_tool_calls={policy.max_tool_calls}.",
                agent=agent,
                mode=mode,
                code="max_tool_calls_exceeded",
...
class PythonDirectEngine(PythonDirectProvider):
    """Backward-compatible runtime alias during the migration."""
...
def _run_calls(agent: Any, tools: Mapping[str, Tool], calls: list[dict[str, Any]], policy: RunPolicy, *, mode: str) -> RunResult:
    ...
    for index, call in enumerate(calls):
        ...
        result = tool.run(call.get("input"), context={"engine": PythonDirectProvider.name, "agent": getattr(agent, "name", ""), "index": index})
...
def _run_pipeline(
    ...
    *,
    ...
    for index, name in enumerate(names):
        ...
        result = tool.run(current, context={"engine": PythonDirectProvider.name, "agent": getattr(agent, "name", ""), "index": index})
        events.extend(result.tool_events)
        output_data = result.data if isinstance(result.data, dict) else {"value": result.data}
...
def _parse_plan(input_value: Any, tools: Mapping[str, Tool]) -> list[dict[str, Any]]:
    if isinstance(input_value, str):
        ...
        if not input_value:
            raise ValueError("PythonDirectProvider received an empty string. Pass a structured tool plan or a direct payload for one tool.")
        try:
            ...
            raise ValueError(
                "PythonDirectProvider is deterministic and does not parse natural language plans. "
    ...
    if isinstance(input_value, Mapping):
        ...
        raise ValueError(
            "PythonDirectProvider needs a tool name when the agent exposes multiple tools. "
    ...
    raise ValueError("Unsupported PythonDirectProvider input. Pass a dict plan, list of plans, JSON string, or single-tool payload.")
...
__all__ = ["PythonDirectEngine", "PythonDirectProvider"]
