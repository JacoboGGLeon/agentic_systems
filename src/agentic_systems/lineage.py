...
LineageKind = Literal["input", "decision", "tool", "validation", "answer", "error", "context"]
...
def _derive_support_reasons(steps: list["LineageStep"], *, ok: bool) -> list[str]:
    ...
    if tool_steps:
        ...
        for step in tool_steps:
            name = step.source or step.title.replace("Tool:", "").replace("Tool evidence:", "").strip()
    ...
    if not reasons:
        reasons.append("La respuesta final conserva evidencia asociada en el RunResult o estado del sistema.")
...
def _evidence_line(step: "LineageStep", *, max_chars: int) -> str:
    ...
    if facts:
        compact_facts = {key: value for key, value in facts.items() if not _is_missing(value)}
        if compact_facts:
...
def _tool_output_facts(output: dict[str, Any], *, max_rows: int = 3) -> dict[str, Any]:
    ...
    if isinstance(rows, list):
        facts["row_count"] = output.get("row_count") or output.get("n_rows") or len(rows)
        facts["sample_rows"] = rows[:max_rows]
    elif isinstance(output.get("data"), dict) and isinstance(output["data"].get("rows"), list):
        rows = output["data"]["rows"]
        facts["row_count"] = output["data"].get("row_count") or output["data"].get("n_rows") or len(rows)
        facts["sample_rows"] = rows[:max_rows]
...
def _business_tool_evidence_from_payload(*payloads: Any) -> dict[str, Any]:
    ...
    if rows is None and isinstance(table.get("rows"), list):
        rows = table.get("rows")
    sections = merged.get("sections") if isinstance(merged.get("sections"), list) else []
    if not sql:
        for section in sections:
            if isinstance(section, dict) and section.get("kind") == "sql" and section.get("content"):
                sql = section.get("content")
                break
    if rows is None:
        for section in sections:
            if isinstance(section, dict) and section.get("kind") == "table" and isinstance(section.get("rows"), list):
    ...
    return {
        ...
        "summary": summary,
        "sql": sql,
        "row_count": len(rows) if isinstance(rows, list) else merged.get("row_count") or merged.get("n_rows"),
...
class LineageMemory(BaseModel):
    ...
    def from_run_result(
        ...
        *,
        ...
        if not hasattr(result, "normalized"):
            raise TypeError("LineageMemory.from_run_result expects a RunResult-like object with normalized().")

        normalized = result.normalized()
        runtime = normalized.get("runtime") or {}
        input_payload = normalized.get("input")
        answer_payload = normalized.get("answer") or {}
        final_payload = answer_payload.get("final") or getattr(result, "final", {}) or {}
        text_answer = str(answer_payload.get("text") or getattr(result, "text", "") or "").strip()
        ...
        resolved_question = question if question is not None else _safe_json(input_payload, max_chars=320)
        steps: list[LineageStep] = [
            LineageStep(
                step_id="input",
                kind="input",
                title="Input received",
                summary=_short(resolved_question, max_chars=280) or "No input recorded.",
        ...
        tools = list(getattr(result, "tool_events", []) or [])
        ...
        if not tools:
            business_evidence = _business_tool_evidence_from_payload(final_payload, getattr(result, "data", {}) or {})
            if business_evidence:
                tool_name = business_evidence.get("tool") or "portable_output"
                route = business_evidence.get("route") or business_evidence.get("query_id") or "portable output"
                row_count = business_evidence.get("row_count")
                summary = str(business_evidence.get("summary") or f"{tool_name} produced portable business evidence via {route}.")
        ...
        if validation:
            ...
            steps.append(
                LineageStep(
                    ...
                    summary="Contract validation passed." if ok else "Contract validation reported issues.",
                    source="RunResult.validation",
                    why="El contrato o policy deja explícito qué se esperaba de la ejecución.",
                    evidence=validation if isinstance(validation, dict) else {"validation": validation},
                )
            )
        ...
        for index, error in enumerate(errors, start=1):
            steps.append(
                LineageStep(
                    ...
                    summary=_short(error.get("message") or error.get("code") or error, max_chars=260) if isinstance(error, dict) else _short(error, max_chars=260),
        ...
        steps.append(
            LineageStep(
                ...
                evidence={"final": final_payload, "data": answer_payload.get("data") or getattr(result, "data", {})},
            )
        )
    ...
    def explain(self) -> dict[str, Any]:
        ...
        for step in self.steps:
            ...
            if tool_steps and step.kind == "decision" and step.title.startswith("Graph node:"):
                continue
            path_steps.append(step)
        evidence_steps = [step for step in self.steps if step.kind in {"decision", "tool", "validation"}]
        ...
        return {
            ...
            "how_it_happened": how_items or [_natural_summary(step.summary) for step in fallback_steps] or ["No executable step was recorded."],
            "why_this_answer": support_reasons,
            "support": support_reasons,
            "validation": self.validation,
            "risks_or_gaps": [_natural_summary(step.summary) for step in error_steps] or [],
    ...
    def human_text(
        ...
        *,
        ...
        support_items = list(explanation.get("support") or explanation.get("why_this_answer") or [])
        ...
        if render_mode == "compact":
            ...
            if support_items:
                lines.extend(["", "Soporte:", f"- {' · '.join(_short(item, max_chars=90) for item in support_items[:3])}"])
            return "\n".join(lines)
        ...
        if how_items:
            for index, item in enumerate(how_items[:max_how_items], start=1):
                lines.append(f"{index}. {_short(_natural_summary(item), max_chars=360)}")
        ...
        if include_evidence:
            evidence_steps = [step for step in self.steps if step.kind in {"decision", "tool", "validation", "answer"}]
            if evidence_steps:
                lines.extend(["", "Evidencia:"])
                for step in evidence_steps[:max_evidence_items]:
                    lines.append(f"- {_evidence_line(step, max_chars=max_evidence_chars)}")
                remaining_evidence = len(evidence_steps) - max_evidence_items
                if remaining_evidence > 0:
                    lines.append(f"- … {remaining_evidence} evidencia(s) más disponibles en `lineage.steps`.")

        if render_mode == "debug":
            lines.extend(["", "Debug lineage payload:", _safe_json(self.compact(), max_chars=max_evidence_chars * 4)])
    ...
    def to_prompt_context(self, *, max_chars: int = 1600) -> str:
        ...
        for step in self.steps:
            evidence = step.evidence.get("facts") if isinstance(step.evidence, dict) else None
            evidence_text = f" | evidence={_safe_json(evidence, max_chars=220)}" if evidence else ""
            lines.append(f"- {step.kind}:{step.title}: {_short(step.summary, max_chars=240)}{evidence_text}")
        text = "\n".join(line for line in lines if line)
        if len(text) <= max_chars:
            return text
        return text[: max(0, max_chars - 3)] + "..."

    def estimated_context_savings(self, raw: Any, *, max_chars: int = 1600) -> dict[str, Any]:
