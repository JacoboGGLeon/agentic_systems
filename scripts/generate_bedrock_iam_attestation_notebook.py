...
Objetivo: certificar el wheel exacto de Agentic Systems 2.1.2 con el modo de
...
| AGENTIC_SYSTEMS_COMMIT_SHA | candidato 2.1.2 | Commit que produjo el wheel. |
| AGENTIC_SYSTEMS_WHEEL_SHA256 | candidato 2.1.2 | Hash del wheel esperado. |
...
if loaded_package is not None and getattr(loaded_package, "__version__", None) != "2.1.2":
    raise RuntimeError(
        ...
        "El wheel 2.1.2 ya se instaló: reinicia el kernel y ejecuta Run All."
    )"""
        ),
        nbformat.v4.new_code_cell(
            """import boto3
...
if toolkit.__version__ != "2.1.2" or missing_api:
    raise RuntimeError(
